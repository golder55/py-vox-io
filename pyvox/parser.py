...
class Chunk(object):
    def __init__(self, id, content=None, chunks=None):
        ...
        if id == b'MAIN':
            ...
            self.voxels = []
            self.voxels = [Voxel(*unpack('BBBB', content, 4+4*i))
                           for i in range(n)]
        elif id == b'RGBA':
            self.palette = [Color(*unpack('BBBB', content, 4*i))
                            for i in range(255)]
            ...
            pass  # raise ParsingException('Unknown chunk type: %s'%self.id)
    ...
    def parse(self):
        ...
        if version != 150:
            raise ParsingException(
                "Unknown vox version: %s expected 150" % version)
        ...
        models = [self._parseModel(chunks.pop(), chunks.pop())
